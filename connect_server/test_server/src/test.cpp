...
public:
	...
	int32 	connect_server(const char* svaddr, int svport){
                ...
                if(m_fd == EF_INVALID_SOCKET || consuc < 0){
                        std::cout << "connect to:" 
	...
	int32	test_register(int32 type, const std::string& id, LoginResponse& lgresp){
		int32 ret = 0;
		...
		std::string rsp;
		ret = do_request(LOGIN_REQ, req, rsp);
...
int main(int argc, const char** argv){
	...
	ret = cli.test_register(atoi(argv[3]), (argv[4]), lgresp);
	if(ret >= 0)
		cli.run();
	return	0;
}
