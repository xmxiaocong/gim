...
RedisMsgDB::RedisMsgDB(const Json::Value &config)
{
	m_cg = NULL;
	m_cfg = config;
...
int RedisMsgDB::bindCG(RedisCG *cg)
{
        if (m_cg == NULL) {
                m_cg = cg;
        }
...
int RedisMsgDB::size(const string &mbName)
{
	int64 size = 0;
...
int RedisMsgDB::sizeFrom(const string &mbName, int64 bMsgId)
{
	int64 count = 0;
	DBHandle hndl = m_cg->getHndl(mbName);
	if (hndl == NULL) return CONNECT_CACHE_FAILED;
...
int RedisMsgDB::getMsgs(const string &mbName, int64 bMsgId, int length, vector<Message> &vMsg)
{
...
int RedisMsgDB::getMsgsForward(const string &mbName, int64 bMsgId, int length, vector<Message> &vMsg)
{
	if (length == 0) return -1;
...
int RedisMsgDB::addMsg(const string &mbName, Message &message)
{
        DBHandle hndl = m_cg->getHndl(mbName);
        if (hndl == NULL) return CONNECT_CACHE_FAILED;
...
int RedisMsgDB::delMsg(const string &mbName, int64 msgId)
{
        DBHandle hndl = m_cg->getHndl(mbName);
        if (hndl == NULL) return CONNECT_CACHE_FAILED;
...
int RedisMsgDB::delMsgsBackward(const string &mbName, int64 bMsgId, int length)
{
	if (length == 0) return -1;
...
int RedisMsgDB::delMsgs(const string &mbName, int64 bMsgId, int length)
{
	if (length == 0) return -1;
...
int RedisMsgDB::clear(const string &mbName)
{
        DBHandle hndl = m_cg->getHndl(mbName);
        if (hndl == NULL) return CONNECT_CACHE_FAILED;
...
int RedisMsgDB::getMsgId(const string &mbName, int64 &recentReadId)
{
        DBHandle hndl = m_cg->getHndl(mbName);
        if (hndl == NULL) return CONNECT_CACHE_FAILED;
...
int RedisMsgDB::setMsgId(const string &mbName, int64 recentReadId)
{
        DBHandle hndl = m_cg->getHndl(mbName);
        if (hndl == NULL) return CONNECT_CACHE_FAILED;
...
int RedisMsgDB::incrId(const string &mbName, int64 &newId)
{
        DBHandle hndl = m_cg->getHndl(mbName);
        if (hndl == NULL) return CONNECT_CACHE_FAILED;
