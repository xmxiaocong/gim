 ...
 * the data and converts bytes int32o longwords for this routine.
 */
 ...
 */
void
MD5Update(MD5_CTX *ctx, uint8 const *buf, uint32 len)
{
	uint32 t;
 ...
 */
void
MD5Final(uint8 digest[16], MD5_CTX *ctx)
{
	uint32 count;
	uint8 *p;
...
void MD5Hex(std::string& hex, uint8 const *buf, uint32 len){
	...
	MD5(digest, buf, len);
	hex.resize(32);
	bytes_to_hexs((const char*)digest, 16, (char*)hex.data(), 32);
}

int32 file_md5 (char const * fname, uint8 * md5val)
{
	MD5_CTX ctx;
